...
<head>
    ...
    <title>{{ $profile->name ?? 'QA Engineer' }} | Portfolio</title>
...
@else
    <nav id="navbar">
        <div class="container nav-content">
            ...
            <a href="mailto:{{ $profile->email }}" class="cta-button">Contact Me</a>
        </div>
    </nav>
    ...
    <header class="hero">
        <div class="container hero-content">
            ...
            <div style="display: flex; gap: 1.5rem;">
                ...
                @if($profile->github)
                    <a href="{{ $profile->github }}" class="glass-card" style="padding: 1rem 2rem; text-decoration: none; color: white;">Github</a>
                @endif
                @if($profile->resume)
                    <a href="{{ asset('storage/' . $profile->resume) }}" target="_blank" class="glass-card" style="padding: 1rem 2rem; text-decoration: none; color: white; border-color: var(--accent-secondary);">Download Resume</a>
                @endif
            </div>
        </div>
    </header>
                        ...
                        @foreach($skillList as $skill)
                            <div class="skill-item">
                                <div class="skill-info">
                                    ...
                                    <span style="color: var(--text-dim);">{{ $skill->proficiency * 10 }}%</span>
                                </div>
                                <div class="proficiency-bar">
                                    <div class="proficiency-fill" style="width: {{ $skill->proficiency * 10 }}%"></div>
                                </div>
                            </div>
                        @endforeach
                    </div>
            ...
            <div class="projects-grid">
                @foreach($projects as $project)
                    <div class="glass-card project-card">
                        ...
                        @endif
                        <h3>{{ $project->title }}</h3>
                        <p style="color: var(--text-dim); font-size: 0.95rem;">{{ $project->description }}</p>
                        <div class="project-tags">
                            @foreach(explode(',', $project->tools_used) as $tool)
                                <span class="tag">{{ trim($tool) }}</span>
                            @endforeach
                        </div>
                        @if($project->link)
                            <a href="{{ $project->link }}" style="color: var(--accent-secondary); text-decoration: none; font-size: 0.9rem; font-weight: 600;">Explore Repository →</a>
                        @endif
                    </div>
                @endforeach
            </div>
            ...
            <div style="max-width: 800px; margin: 0 auto;">
                @foreach($experiences as $exp)
                    <div class="experience-item">
                        ...
                        <p style="color: var(--accent-primary); font-weight: 600; margin-bottom: 1rem;">{{ $exp->company }}</p>
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 2rem;">
                @foreach($certifications as $cert)
                    <div class="glass-card" style="padding: 1.5rem;">
                        ...
                        <p style="color: var(--accent-secondary); font-size: 0.9rem; margin-bottom: 0.5rem;">{{ $cert->issuing_organization }}</p>
                        @if($cert->issue_date)
                            <p style="color: var(--text-dim); font-size: 0.8rem;">{{ \Carbon\Carbon::parse($cert->issue_date)->format('M Y') }}</p>
                        @endif
                    </div>
                @endforeach
            </div>
